...
class Solution:
    def invertTree(self, root: Optional[TreeNode]) -> Optional[TreeNode]:
        ...
        # # With depth first search using stack
        # if root == None:
        #     return root
        # rootList = [root]
        # while rootList:
        ...
        # With breadth first search using queue
        if root == None:
            return root
        rootQueue = deque([root])
        while rootQueue:
            currentRoot = rootQueue.popleft()
            if currentRoot.left:
                rootQueue.append(currentRoot.left)
            if currentRoot.right:
                rootQueue.append(currentRoot.right)
            tmpLeft = currentRoot.left
            tmpRight = currentRoot.right
            currentRoot.left = tmpRight
            currentRoot.right = tmpLeft
